...
public class Money {
  public static int calculateYears(double principal, double interest,  double tax, double desired) {
    ...
    while (balance < desired) {
      balance += getInterestWithoutTax(interest, tax, balance);
      years++;
    }
    return years;
  }

  private static double getInterestWithoutTax(final double interest, final double tax, final double balance) {
    return (balance * interest) - ((balance * interest)* tax);
  }
}
